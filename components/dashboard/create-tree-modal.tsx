...
import { FieldFrame } from '@/components/ui/studio';
...
export function CreateTreeModal({ action }: { action: (formData: FormData) => void | Promise<void> }) {
  ...
  return (
    <>
      ...
      <dialog ref={modalRef} className="w-[calc(100%-2rem)] max-w-md rounded-lg border border-border bg-white p-0 shadow-2xl backdrop:bg-slate-950/35 backdrop:backdrop-blur-sm">
        <form action={action} className="space-y-4 p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="text-xl font-semibold text-slate-950">Create family tree</h2>
          ...
          <FieldFrame label="Tree name">
            <input required name="name" className="studio-field" placeholder="Rivera family" />
          </FieldFrame>
          <FieldFrame label="Description">
            <textarea name="description" className="studio-field min-h-28 resize-none" placeholder="Who this workspace is for, and what relatives should know." />
          </FieldFrame>

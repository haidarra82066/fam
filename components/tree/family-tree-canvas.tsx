...
function CanvasInner(props:any){ const rf=useReactFlow(); const [selected,setSelected]=useState<any>(null); const [horizontal,setHorizontal]=useState(false); const [depth,setDepth]=useState(99); const [query,setQuery]=useState(''); const prepared=useMemo(()=>layout(props.persons,props.unions,props.parentChild,horizontal),[props.persons,props.unions,props.parentChild,horizontal]); const nodes=prepared.nodes.filter((n:any)=>{const p=n.data.person; return p.display_name.toLowerCase().includes(query.toLowerCase()) && (horizontal || n.position.y/220<=depth);}).map((n:any)=>({...n,data:{label:<div className="rounded-full border bg-white px-4 py-2 text-center text-xs"><div className="font-medium">{n.data.person.display_name}</div><div className="text-muted">{years(n.data.person)}</div><div>{n.data.person.living_status==='deceased'?'✝':''} {n.data.person.is_private?'🔒':''}</div></div>}}));

...
export function ShareModal({ treeId, action, latestInviteToken }: Props) {
  ...
  return <>
    ...
    {open ? <div className="fixed inset-0 z-50 grid place-items-end bg-slate-950/40 p-0 backdrop-blur-sm sm:place-items-center sm:p-4">
      <div className="max-h-[92dvh] w-full max-w-lg overflow-y-auto rounded-t-xl bg-white p-5 pb-[calc(1.25rem+env(safe-area-inset-bottom))] shadow-2xl sm:rounded-xl sm:pb-5">
        ...
        <div className="mb-5 flex items-start justify-between gap-3">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold tracking-tight text-slate-950">Share this tree</h2>
        ...
        <form action={action} className="space-y-3 rounded-xl border border-[#dfe9e7] bg-[#f8fbfa] p-3">
          <input type='hidden' name='tree_id' value={treeId} />
          <label className="block text-sm font-medium text-slate-700">
            Email
            <input name="email" type="email" autoComplete="email" required placeholder="family@example.com" className="mt-1 min-h-11 w-full rounded-lg border border-border bg-white px-3 py-2 text-sm outline-none focus:border-accent" />
          </label>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-[1fr_130px]">
            <label className="block text-sm font-medium text-slate-700">
              Role
              <select name="role" className="mt-1 min-h-11 w-full rounded-lg border border-border bg-white px-3 py-2 text-sm capitalize outline-none focus:border-accent" defaultValue="viewer">
                <option value='viewer'>viewer</option><option value='contributor'>contributor</option><option value='editor'>editor</option>
              </select>
            </label>
            <label className="block text-sm font-medium text-slate-700">
              Expires
              <input name="expires_days" aria-label="Invite expiration days" type="number" min={1} max={30} defaultValue={7} className="mt-1 min-h-11 w-full rounded-lg border border-border bg-white px-3 py-2 text-sm outline-none focus:border-accent" />
            </label>
          </div>
        ...
        <div className="mt-4 grid gap-2 text-xs text-muted sm:grid-cols-3">{Object.entries(roleDescriptions).map(([role, text]) => (
          <div key={role} className="rounded-lg border border-border bg-white p-3">
            <p className="font-semibold capitalize text-slate-900">{role}</p>
            <p className="mt-1 leading-5">{text}</p>
          </div>
        ))}</div>
        {inviteLink ? <div className="mt-4 space-y-3 rounded-xl border border-border p-3">
          <p className="text-sm font-semibold text-slate-900">Invite link</p>
          <input readOnly value={inviteLink} className="min-h-11 w-full rounded-lg border border-border bg-slate-50 px-3 py-2 text-xs outline-none" />
